...
// ── 6. My Current Workload (staff) ────────────────
const getMyWorkload = catchAsync(async (req, res) => {
  if (!req.user.department) throw ApiError.badRequest('No department assigned');
  const data = await analyticsService.getStaffCurrentWorkload(req.user._id, req.user.department);
  ApiResponse.success(res, data, 'Current workload');
});
