...
const checkIn = catchAsync(async (req, res) => {
  const result = await apptService.checkInAppointment(req.params.id, {
...
// Staff/Admin: full dashboard board — pending (any date) / today / upcoming / completed / cancelled.
// This is what the staff dashboard should call instead of getDeptAppointments, since
// getDeptAppointments is locked to a single exact date and can't represent "upcoming".
const getDeptBoard = catchAsync(async (req, res) => {
  const board = await apptRepo.findDepartmentBoard(req.params.departmentId, {
...
module.exports = { getSlots, book, uploadDocuments, reviewDocument, getMyAppointments, getOne, checkIn, cancel,
                   getDeptAppointments, getDeptBoard, approve, reject };