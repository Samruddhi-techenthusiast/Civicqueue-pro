...
const markServed = async (tokenId, staffId) => {
  ...
  // Sync back to the originating Appointment, if this token came from a check-in.
  // Without this, Appointment.status stays at 'checked_in' forever — it can never
  // reach 'completed', which is exactly why the dashboard's "Completed" bucket
  // was always empty regardless of how many citizens were actually served.
  if (token.appointmentRef) {
    await Appointment.findByIdAndUpdate(token.appointmentRef, { status: 'completed' }).catch((err) =>
      logger.warn('Failed to sync appointment status to completed', { appointmentRef: token.appointmentRef, err: err.message })
    );
  }
...
const cancelToken = async (tokenId, cancelledBy, cancelReason) => {
  ...
  if (token.appointmentRef) {
    await Appointment.findByIdAndUpdate(token.appointmentRef, {
      status: 'cancelled', cancelledAt: new Date(), cancelReason: cancelReason || 'Cancelled',
    }).catch((err) =>
      logger.warn('Failed to sync appointment status to cancelled', { appointmentRef: token.appointmentRef, err: err.message })
    );
  }
...
const skipToken = async (tokenId, staffId, reason) => {
  ...
  if (token.appointmentRef) {
    await Appointment.findByIdAndUpdate(token.appointmentRef, {
      status: 'no_show', cancelReason: reason || 'No show / skipped',
    }).catch((err) =>
      logger.warn('Failed to sync appointment status to no_show', { appointmentRef: token.appointmentRef, err: err.message })
    );
  }
...
const recallToken = async (tokenId, staffId, counter) => {
  ...
  if (token.appointmentRef) {
    await Appointment.findByIdAndUpdate(token.appointmentRef, { status: 'checked_in' }).catch((err) =>
      logger.warn('Failed to sync appointment status on recall', { appointmentRef: token.appointmentRef, err: err.message })
    );
  }
