  ...
  const result = await Token.aggregate([
    ...
    },
    {
      $group: {
        _id: '$servedBy',
        tokensServed: { $sum: 1 },
        // Tokens that originated from a pre-booked appointment check-in, as opposed
        // to a walk-in — this is the real "appointments handled" count per staff.
        appointmentsHandled: { $sum: { $cond: [{ $ne: ['$appointmentRef', null] }, 1, 0] } },
        avgHandlingMs: {
...
/**
 * Real-time workload for a single staff member — what they're doing RIGHT NOW,
 * not a historical stat. Used by the staff dashboard to show "current workload"
 * per the requirements: the token they're actively serving (if any) plus how
 * many people are waiting in their department's queue at this moment.
 */
const getStaffCurrentWorkload = async (staffId, departmentId) => {
  const [currentlyServing, waitingCount] = await Promise.all([
    Token.findOne({ servedBy: staffId, status: 'serving' })
      .select('tokenNumber citizenName servingStartedAt counter')
      .lean(),
    Token.countDocuments({ department: departmentId, status: 'waiting' }),
  ]);
  return { currentlyServing: currentlyServing || null, waitingInQueue: waitingCount };
};


const getDeptPerformanceComparison = async (fromDate, toDate) => {
  const cacheKey = `analytics:dept_compare:${fromDate}:${toDate}`;
  const cached = await cache.get(cacheKey);
  if (cached) return cached;

  const [deptStats, depts] = await Promise.all([
    Token.aggregate([
      { $match: { date: { $gte: fromDate, $lte: toDate } } },
      {
        $group: {
          ...
          cancelled:{ $sum: { $cond: [{ $eq: ['$status', 'cancelled'] }, 1, 0] } },
          avgWaitMs: { $avg: { $subtract: ['$servedAt', '$issuedAt'] } },
        },
      },
    ]),
  ...
  const result = deptStats.map(s => {
    ...
    return {
      ...
      serviceRate: s.total ? Math.round((s.served / s.total) * 100) : 0,
      noShowRate: s.total ? Math.round((s.noShow / s.total) * 100) : 0,
      avgWaitMinutes: s.avgWaitMs ? Math.round(s.avgWaitMs / 60000) : 0,
    };
...
module.exports = { getDepartmentAnalytics, getAdminOverview, getWeeklyOverview, getStaffPerformance, getStaffCurrentWorkload, getDeptPerformanceComparison }
