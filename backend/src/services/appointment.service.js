...
// `actor` is whoever is performing the check-in: the citizen themselves (self
// check-in from the app), or a staff/admin member marking someone as arrived
// at the counter. Only citizens are restricted to their own appointment;
// staff are restricted to their own department instead (checked below).
const checkInAppointment = async (appointmentId, actor) => {
  const isStaffActor = ['staff', 'admin', 'super_admin'].includes(actor.role);
  ...
  if (!isStaffActor) matchFilter.citizen = actor.id; // citizens can only check in their own
  ...
  if (!appt) {
    ...
    if (!isStaffActor && existing.citizen.toString() !== actor.id.toString())
  ...
  // Staff can only check in appointments in their own department (admins bypass this)
  if (actor.role === 'staff' && actor.department?.toString() !== appt.department.toString()) {
    await Appointment.findByIdAndUpdate(appointmentId, { $set: { checkInLock: false } })
    throw ApiError.forbidden('Cannot check in appointments for another department')
  }
...
module.exports = { getAvailableSlots, bookAppointment, checkInAppointment, cancelAppointment, approveAppointment, rejectAppointment };
