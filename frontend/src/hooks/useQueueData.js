...
export function useQueueData(departmentId, pollInterval = 5000) {
  ...
  const [appointments, setAppointments] = useState({ pending: [], today: [], upcoming: [], completed: [], cancelled: [] })
  ...
  const fetchAppointments = useCallback(async () => {
    if (!departmentId) return
    try {
      const { data } = await api.get(`/appointments/department/${departmentId}/board`)
      if (mountedRef.current) setAppointments(data.data || { pending: [], today: [], upcoming: [], completed: [], cancelled: [] })
    } catch { /* non-critical */ }
  }, [departmentId])
  ...
  // Alias — same endpoint, but this is the staff-facing "mark this citizen as
  // arrived at the counter" action (as opposed to a citizen checking themself in
  // from their own device). The backend now supports both from the same route.
  const markArrived = checkInAppointment
